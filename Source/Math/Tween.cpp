...
namespace NinjaParty
{
	...
	Tween::Tween(float tweenDuration, TweenType tweenType, TweenFunction tweenFunction)
	{
		position = 0;
	...
	void Tween::Reset(float tweenDuration, TweenType tweenType, TweenFunction tweenFunction)
	{
		position = 0;
		duration = tweenDuration;
		function = tweenFunction;
	...
	float Tween::Update(float deltaSeconds)
	{
		position += deltaSeconds;
		return std::min<float>(1.0f, type(position / duration, function));
	}
}
